...
function getStatusMessage(data) {
  ...
  if (data.status === "BLOOD_BANK_AVAILABLE") {
    return "Donor unavailable, but blood stock is available from hospitals and blood banks.";
  }
...
async function loadTracking() {
  ...
  try {
    ...
    showToast(error.message || "Failed to load tracking", "error");
    trackContainer.innerHTML = `<p class="track-error">${error.message || "Error loading tracking info"}</p>`;
  ...
  stockContainer.innerHTML = `
    <div class="auth-neo" style="margin-top:18px;">
      ...
      <div class="card-head" style="margin-top:18px;">
        ...
        <h3>Blood Banks and Nearby Sources</h3>
